...
export function Dashboard() {
  ...
  const { data: monthlyStats = [], isLoading: monthlyStatsLoading } =
    useDashboardMonthly();
  const { data: farmStats = [], isLoading: farmStatsLoading } =
    useDashboardFarmStats();
  ...
  const recentMonths = [...monthlyStats]
    .sort((a, b) => b.month.localeCompare(a.month))
    .slice(0, 4);
  const leadingFarms = [...farmStats]
    .sort((a, b) => b.totalValue - a.totalValue)
    .slice(0, 4);
  const topProduct = [...productStats].sort(
    (a, b) => b.totalValue - a.totalValue,
  )[0];
  const latestTrend = [...trends].sort((a, b) =>
    ...
    <div className="space-y-stack-lg animate-in fade-in duration-300">
      ...
      <section className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-secondary-container text-on-secondary-container rounded-full border border-on-secondary-container/10">
          ...
          <span className="text-xs font-bold">
            {summary?.totalFarms ?? 0} fincas • {summary?.totalLots ?? 0} lotes
            conectados
          </span>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="flex flex-col justify-between">
          ...
          <div className="mt-4">
            <p className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">
              Valor Estimado Bruto
            </p>
            <p className="text-2xl font-black text-on-surface mt-1 truncate">
              {formattedEarnings}
            </p>
          </div>
        </Card>
        ...
        <div className="lg:col-span-8 relative min-h-64 rounded-2xl overflow-hidden group border border-[#EEFFCD]">
          ...
          <div className="absolute bottom-0 left-0 p-6 space-y-3 z-10">
            ...
            <h3 className="text-white text-2xl font-black tracking-tight">
              {topProduct
                ? `${topProduct.productName} concentra el mejor valor estimado`
      ...
      <section className="bg-surface-lowest rounded-2xl border border-[#EEFFCD] p-6 shadow-[0px_4px_20px_rgba(30,41,59,0.05)] space-y-6">
        ...
        {recentHarvests.length === 0 ? (
          <div className="py-6 text-center text-on-surface-variant font-medium text-sm">
            No hay cosechas registradas todavía. ¡Comience tocando 'Registrar
            Nueva Cosecha'!
          </div>
        ) : (
              ...
              >
                ...
                <div className="text-right ml-4">
                  ...
                  <p className="text-xs font-semibold text-on-surface-variant mt-0.5">
                    {(h.estimatedWeight / 1000).toFixed(1)} Tons ({h.quantity}{" "}
                    Racimos)
                  </p>
                </div>
              </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-on-surface-variant font-medium">
                Tendencia calculada desde /api/statistics/monthly.
              </p>
            </div>
            <span className="px-3 py-1 rounded-full bg-primary-container/40 text-primary text-xs font-bold">
              {monthlyStats.length} periodos
            </span>
          </div>

          {recentMonths.length === 0 ? (
            <div className="py-8 text-center text-sm font-medium text-on-surface-variant">
              Aún no hay producción mensual agregada en la API.
            </div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant font-medium mt-1">
                        {month.totalHarvests} cosechas • {month.totalBunches.toLocaleString("es-CO")} racimos
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-black text-primary">
                        {(month.totalWeightKg / 1000).toFixed(2)} Tons
                      </p>
                      <p className="text-xs text-on-surface-variant font-medium mt-1">
                        {formatCOP(month.totalValue)}
                      </p>
                    </div>
                  </div>
                    ...
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          ...
                          Math.max(
                            8,
                            summary?.totalWeightEstimated
                              ? (month.totalWeightKg / summary.totalWeightEstimated) * 100
                              : 8,
                          ),
                        )}%`,
                      }}
        ...
        <Card className="space-y-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-on-surface-variant font-medium">
                Consolidado real desde /api/statistics/farms.
              </p>
            </div>
            <button
          ...
          {leadingFarms.length === 0 ? (
            <div className="py-8 text-center text-sm font-medium text-on-surface-variant">
              Registre cosechas para generar comparativos por finca.
            </div>
          ) : (
            <div className="space-y-3">
              {leadingFarms.map((farm, index) => (
                <div
                  key={farm.farmId}
                  className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-4 flex items-center justify-between gap-4"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center text-sm font-black">
                        {index + 1}
                      </div>
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-on-surface-variant font-medium mt-1">
                          {farm.totalHarvests} cosechas registradas
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-black text-secondary">
                      {formatCOP(farm.totalValue)}
                    </p>
                    <p className="text-xs text-on-surface-variant font-medium mt-1">
                      {(farm.totalWeightKg / 1000).toFixed(2)} Tons
                    </p>
                  </div>
                </div>
              ))}
